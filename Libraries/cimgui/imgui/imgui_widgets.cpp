...
bool ImGui::ButtonBehavior(const ImRect &bb, ImGuiID id, bool *out_hovered, bool *out_held, ImGuiButtonFlags flags) {
    ...
    if (g.NavActivateDownId == id) {
        ...
        bool nav_activated_by_inputs = IsNavInputTest(ImGuiNavInput_Activate,
                                                      (flags & ImGuiButtonFlags_Repeat) ? ImGuiNavReadMode_Repeat
...
bool ImGui::BeginComboPreview() {
    ...
    if (window->SkipItems || !window->ClipRect.Overlaps(
            g.LastItemData.Rect)) // FIXME: Because we don't have a ImGuiItemStatusFlags_Visible flag to test last ItemAdd() result
        return false;
...
static const char *PatchFormatStringFloatToInt(const char *fmt) {
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f') {
        ...
        ImFormatStringToTempBuffer(&tmp_format, NULL, "%.*s%%d%s", (int) (fmt_start - fmt), fmt,
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE *v, float v_speed, const TYPE v_min, const TYPE v_max,
    ...
    if (g.ActiveIdSource == ImGuiInputSource_Mouse && IsMousePosValid() &&
        ...
        adjust_delta = GetNavInputAmount2d(ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad,
                                           ImGuiNavReadMode_RepeatFast, 1.0f / 10.0f, 10.0f)[axis];
    ...
    if (g.ActiveId == id) {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse) {
            ...
            const ImVec2 input_delta2 = GetNavInputAmount2d(
                    ImGuiNavDirSourceFlags_Keyboard | ImGuiNavDirSourceFlags_PadDPad, ImGuiNavReadMode_RepeatFast, 0.0f,
                    0.0f);
...
bool ImGui::InputTextEx(const char *label, const char *hint, char *buf, int buf_size, const ImVec2 &size_arg,
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_shortcut_key = g.IO.ConfigMacOSXBehaviors ? (io.KeyMods == ImGuiModFlags_Super) : (io.KeyMods ==
                                                                                                         ImGuiModFlags_Ctrl);
        ...
        const bool is_validate_nav =
                (IsNavInputTest(ImGuiNavInput_Activate, ImGuiNavReadMode_Pressed) && !IsKeyPressed(ImGuiKey_Space)) ||
                IsNavInputTest(ImGuiNavInput_Input, ImGuiNavReadMode_Pressed);
        const bool is_cancel =
                IsKeyPressed(ImGuiKey_Escape) || IsNavInputTest(ImGuiNavInput_Cancel, ImGuiNavReadMode_Pressed);
...
bool ImGui::TabItemEx(ImGuiTabBar *tab_bar, const char *label, bool *p_open, ImGuiTabItemFlags flags) {
    ...
    bool just_closed;
    bool text_clipped;
    TabItemLabelAndCloseButton(display_draw_list, bb, flags, tab_bar->FramePadding, label, id, close_button_id,
